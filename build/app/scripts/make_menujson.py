...
if __name__ == '__main__':
    ...
    menu_log_fzf = [
        ...
        ("rawstats", f"{tailcmd} {logsdir2}/rawstats | {fzfcmd}"),
        ("peerstats", f"{tailcmd} {logsdir2}/peerstats | {fzfcmd}"),
        ("loopstats", f"{tailcmd} {logsdir2}/loopstats | {fzfcmd}"),
        ("sysstats", f"{tailcmd} {logsdir2}/sysstats | {fzfcmd}"),
        ("clockstats", f"{tailcmd} {logsdir2}/clockstats | {fzfcmd}"),
        ]
    menu_log = [
        (f"{appname} log (syslog)", f"more {logsdir}/{appname}.log"),
        ("rawstats", f"more {logsdir2}/rawstats"),
        ("peerstats", f"more {logsdir2}/peerstats"),
        ("loopstats", f"more {logsdir2}/loopstats"),
        ("sysstats", f"more {logsdir2}/sysstats"),
        ("clockstats", f"more {logsdir2}/clockstats"),
        ]
    menu_configuration = [
        (f"{appname} configuration", "more /etc/ntp.conf"),
